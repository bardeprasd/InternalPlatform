...
export default function ProfileSection() {
  ...
  // same anchor for Chip & Avatar
  const anchorRef = useRef(null);

  const handleToggle = () => setOpen((prevOpen) => !prevOpen);

  const handleClose = (event) => {
    if (anchorRef.current && event?.target && anchorRef.current.contains(event.target)) return;
    setOpen(false);
  };

  const handleLogout = (event) => {
    // optional: clear auth here, e.g. localStorage.removeItem('token');
    setOpen(false);
    navigate('/pages/login'); // route target
  };
